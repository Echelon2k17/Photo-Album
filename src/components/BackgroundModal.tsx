...
export const BackgroundModal: React.FC<BackgroundModalProps> = ({
  ...
  const [customBgUrl, setCustomBgUrl] = useState<string>(
    initialBg.type === 'image' && initialBg.customImageUrl ? initialBg.customImageUrl : ''
  );
  ...
  const buildCurrentConfig = (): PageBackgroundConfig => {
    ...
    if (activeTab === 'image') {
      return {
        type: 'image',
        color: '#f8fafc',
        photoId: customBgUrl ? undefined : selectedPhotoId,
        customImageUrl: customBgUrl || undefined,
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in fade-in zoom-in-95 duration-150">
        ...
        <div className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
          ...
          {/* 4. PHOTO WATERMARK / CUSTOM IMAGE TAB */}
          {activeTab === 'image' && (
            <div className="space-y-4">
              {/* Upload Custom Image / Wallpaper Banner */}
              <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-xl space-y-2.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Upload className="w-4 h-4 text-blue-600" />
                    <span className="text-xs font-bold text-blue-900">Upload Custom Background Image</span>
                  </div>
                  {customBgUrl && (
                    <button
                      type="button"
                ...
                <p className="text-xs text-blue-700">
                  Upload any high-res photo, floral artwork, gradient wallpaper, or texture from your device.
                </p>
                <div className="flex items-center gap-3">
                  <label className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-white border border-blue-300 hover:bg-blue-100/50 rounded-lg text-xs font-bold text-blue-800 cursor-pointer shadow-2xs transition">
                  ...
                  {customBgUrl && (
                    <div className="flex items-center gap-2">
                      <img src={customBgUrl} alt="Custom background" className="w-10 h-10 object-cover rounded border border-blue-300" />
                      <span className="text-xs font-semibold text-emerald-700 flex items-center gap-1">
                        <Check className="w-3.5 h-3.5" /> Active custom background
                      </span>
                    </div>
                  )}
                </div>
              ...
              {/* Or Select from Album Photos */}
              <div className="space-y-2">
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">
                  Or Pick From Album Photos ({albumPhotos.length})
                </span>
                {albumPhotos.length === 0 ? (
                  <p className="text-xs text-slate-500">No album photos available yet.</p>
                ) : (
                  <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 max-h-36 overflow-y-auto p-1">
                    {albumPhotos.map((photo) => {
                      const isSelected = !customBgUrl && selectedPhotoId === photo.id;
                      return (
                        <button
                          key={photo.id}
                          type="button"
